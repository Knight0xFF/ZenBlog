...
def archives():
    ...
    for y in xrange(2014, 2016):
        temp = []
        for p in posts:
            if p.post_date.strftime("%Y") == str(y):
                temp.append(p)
        archive[str(y)] = temp
...
def search(pageid=1):
    ...
    posts = searchresult.order_by(Post.post_date.desc()).paginate(pageid, POSTS_PER_PAGE)
...
def category(category_name, pageid=1):
    ...
    shuffle(tags)
    tags = tags[:10]
    posts = Post.query.filter_by(category_name=category_name).order_by(Post.post_date.desc()).paginate(pageid, POSTS_PER_PAGE)
...
def new_post():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('post-preview') == '1':
            ...
            return render_template('admin-posts-preview.html', post=post_data)
        else:
...
@app.route("/post_hide/<int:post_id>")
...
@app.route('/test/<text>')
def test(text):
    return Response(markdown.markdown(text))